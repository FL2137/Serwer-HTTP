...
int main() {
	Web::TCP::tcp_server::async_server(std::string("192.168.1.103"), 69, [](std::string request_string,std::string& resp) {
		...
		if (request.method == "GET"){
			...
			if (request.url == "/xml") {
				...
				resp = HTTP::send_raw(R"({"r":"pipa","c":"niepipa"})","application/json)");
			}
